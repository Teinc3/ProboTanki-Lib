package §override const package§
{
   ...
   public class §'"]§ implements §else if§
   {
      [Inject]
      ...
      private var §switch package set§:Vector.<§!!5§>;
      
      public function §'"]§(param1:Camera3D)
      {
         this.§switch package set§ = new Vector.<§!!5§>();
         super();
         this.camera = param1;
      }
      ...
      {
         var _loc1_:§!!5§ = null;
         for each(_loc1_ in this.§switch package set§)
         {
            _loc1_.visible = true;
         }
      }
      
      public function §finally var function§(param1:§!!5§) : void
      {
         param1.visible = false;
         battleService.§7#P§().§,#s§(param1);
         this.§switch package set§.push(param1);
      }
      
      public function render(param1:int, param2:int) : void
      {
         var _loc3_:§!!5§ = null;
         var _loc4_:Matrix4 = this.§do set for§();
         for each(_loc3_ in this.§switch package set§)
         {
            this.§final super§(_loc3_,_loc4_);
         }
      }
      
      private function §final super§(param1:§!!5§, param2:Matrix4) : void
      {
         ...
         var _loc5_:Number = this.§1#7§();
         var _loc6_:Boolean = this.§,S§(v.x,v.y,_loc4_,_loc5_);
         if(v.z > 0 && _loc6_)
         {
            _loc3_ = this.§extends package try§(param1.§3#v§());
            if(_loc3_ == 0)
            {
               param1.visible = false;
               param1.alpha = 0;
         ...
         var _loc7_:§0_§ = battleService.§7#P§();
         param1.x = int(v.x + _loc7_.§%!4§() / 2 - 12);
         param1.y = int(v.y + _loc7_.§case catch break§() / 2 - 12);
         param1.update();
      }
      
      private function §extends package try§(param1:§case set extends§) : Number
      {
         var _loc2_:Number = NaN;
         var _loc3_:Number = NaN;
         var _loc4_:RayIntersectionData = null;
         param1.readPosition(§1"!§);
         var _loc5_:Number = §-"4§(this.camera,§1"!§);
         if(_loc5_ < §final true§.value)
         {
            return 1;
         }
         ...
         direction.normalize();
         _loc4_ = battleService.§3l§().raycast(§for package function§,direction,battleService.§^#6§());
         if(_loc4_ != null && _loc4_.time < _loc3_)
         {
            _loc2_ = 1;
         }
         else if(_loc5_ > §-!6§.value)
         {
            _loc2_ = 0;
         }
         else
         {
            _loc2_ = (§-!6§.value - _loc5_) / (§-!6§.value - §final true§.value);
         }
         return _loc2_;
      }
