package alternativa.tanks.battle
{
   import §7"j§.§<#z§;
   import §7"j§.§extends package switch§;
   import §?#Z§.§9!c§;
   import §?#Z§.§return var native§;
   import alternativa.math.Matrix3;
   import alternativa.math.Vector3;
   import alternativa.tanks.battle.objects.tank.Tank;
   import alternativa.tanks.battle.objects.tank.§continue var case§;
   import alternativa.tanks.camera.§3!0§;
   import alternativa.tanks.engine3d.§throw const switch§;
   import alternativa.tanks.utils.GraphicsUtils;
   import alternativa.utils.TextureMaterialRegistry;
   import flash.display.BlendMode;
   import flash.utils.Dictionary;
   import platform.client.fp10.core.resource.types.MultiframeImageResource;
   
   public class §static const super§
   {
      private static const §0S§:Number = 0.2;
      
      private static const §override set class§:Number = 1;
      
      private static const bias:Vector3 = new Vector3(100,0,0);
      
      private static const §=!#§:Vector3 = new Vector3();
      
      private static const §each const continue§:Vector3 = new Vector3();
      
      private var battleService:BattleService;
      
      private var dustSize:Number = 0;
      
      private var animation:§throw const switch§;
      
      private var tanks:Dictionary;
      
      private var camera:§3!0§;
      
      private var nearDistance:Number;
      
      private var farDistance:Number;
      
      public var enabled:Boolean = true;
      
      private var intensity:Number;
      
      private var §default const in§:Number;
      
      public function §static const super§(param1:BattleService)
      {
         this.tanks = new Dictionary();
         super();
         this.battleService = param1;
         this.camera = param1.§3l§().§super package§();
      }
      
      private static function §@"Q§(param1:Vector3, param2:Number) : void
      {
         param1.x += (Math.random() - 0.5) * 2 * param2;
         param1.y += (Math.random() - 0.5) * 2 * param2;
         param1.z += (Math.random() - 0.5) * 2 * param2;
      }
      
      public function init(param1:MultiframeImageResource, param2:TextureMaterialRegistry, param3:Number, param4:Number, param5:Number, param6:Number, param7:Number) : void
      {
         this.animation = GraphicsUtils.getTextureAnimationFromResource(param2,param1);
         this.farDistance = param3;
         this.nearDistance = param4;
         this.dustSize = param5;
         this.intensity = param6;
         this.§default const in§ = param7;
      }
      
      public function §each package import§(param1:Tank) : void
      {
         this.tanks[param1] = param1.§&#[§() / 600;
      }
      
      public function §true set false§(param1:Tank) : void
      {
         delete this.tanks[param1];
      }
      
      public function update() : void
      {
         var _loc1_:* = undefined;
         var _loc2_:Tank = null;
         if(this.enabled && this.camera.softTransparency && this.camera.softTransparencyStrength > 0)
         {
            for(_loc1_ in this.tanks)
            {
               _loc2_ = _loc1_ as Tank;
               if(Boolean(_loc2_) && _loc2_.state == §continue var case§.§?!S§)
               {
                  this.§6"%§(_loc2_,100,this.§default const in§);
               }
            }
         }
      }
      
      public function §6"%§(param1:Tank, param2:Number = 100, param3:Number = 0.2) : void
      {
         var _loc4_:Number = Number(this.tanks[param1]);
         var _loc5_:§return var native§ = param1.§catch set if§();
         var _loc6_:§return var native§ = param1.§!"b§();
         if(_loc5_.§6#§ * _loc6_.§6#§ < 0)
         {
            param2 = 5;
         }
         var _loc7_:Matrix3 = param1.§0"t§().baseMatrix;
         bias.x *= -1;
         _loc7_.transformVector(bias,§=!#§);
         this.§package native§(_loc5_,_loc4_,§=!#§,param2,param3);
         bias.x *= -1;
         _loc7_.transformVector(bias,§=!#§);
         this.§package native§(_loc6_,_loc4_,§=!#§,param2,param3);
      }
      
      private function §package native§(param1:§return var native§, param2:Number, param3:Vector3, param4:Number, param5:Number) : void
      {
         var _loc6_:§9!c§ = null;
         var _loc7_:Number = NaN;
         var _loc8_:Number = NaN;
         var _loc9_:int = 0;
         while(_loc9_ < param1.§@x§)
         {
            _loc6_ = param1.§@"y§[_loc9_];
            if((_loc7_ = _loc6_.speed) > param4 && Math.random() < param5)
            {
               _loc8_ = _loc7_ > 500 ? Number(1) : Number(0.3 + _loc7_ / 712);
               §each const continue§.copy(_loc6_.§5!o§());
               §@"Q§(§each const continue§,50);
               param3.z = 100;
               §@"Q§(param3,20);
               this.§while var implements§(param2,§each const continue§,param3,_loc8_);
            }
            _loc9_++;
         }
      }
      
      private function §while var implements§(param1:Number, param2:Vector3, param3:Vector3, param4:Number) : void
      {
         var _loc5_:§<#z§ = null;
         var _loc6_:§extends package switch§ = null;
         var _loc7_:Number = NaN;
         if(this.enabled && this.camera.softTransparency && this.camera.softTransparencyStrength > 0)
         {
            _loc5_ = §<#z§(this.battleService.§class while§().getObject(§<#z§));
            _loc5_.init(param2,param3,0.01);
            _loc6_ = §extends package switch§(this.battleService.§class while§().getObject(§extends package switch§));
            _loc7_ = this.dustSize * param1 * (1 + §override set class§ * Math.random());
            _loc6_.init(_loc7_,_loc7_,this.animation,Math.random() * 2 * Math.PI,_loc5_,0.5,0.5,null,130,BlendMode.NORMAL,this.nearDistance,this.farDistance,this.intensity * param4 * this.camera.softTransparencyStrength,true);
            this.battleService.§3l§().§7"0§(_loc6_);
         }
      }
   }
}

