...
class BattleProcessor(AbstractProcessor):
    ...
    def __init__(self, holder):
        ...
        self._client_time = 0
        self._last_client_time = Datetime.now()
        self.server_time = 0
    ...
    def client_time(self):
        ...
        self._client_time += int(self.CLIENT_TIME_SPEED_FACTOR * elapsed_time)
        ...
        if self.compare_packet("Start_Resp_Fantom"):
            ...
            if not any(sheep['username'] == username for sheep in self.sheep_list):
                ...
                if self.holder.storage['credentials']['username'] == username:
                    self.current_sheep = self.sheep_list[-1]
            else:
    ...
    def execute_battle_function(self):
        ...
        opposite_sheeps = list(filter(lambda sheep_info: sheep_info['team'] == opposite_team, self.sheep_list))
