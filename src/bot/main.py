...
class TankiBot:
    ...
    def start_cycle(self):
        self.watchdog = TankiSocket(CallbackHolder(
            storage={'credentials': self.credential_manager.get_next_credentials(),
        ...
        self.sheep = []
        max_sheep = min(self.credential_manager.remaining_accounts_count,
                        self.proxy_manager.remaining_connections_count)
    ...
    def sheep_ready(self, sheep_id: int, ready_state: bool):
        ...
        if len(self.sheeps_ready) == self.battle_size:
            ...
            self.event_emitter.emit('all_sheep_ready', {
                ...
                'name': self.SELECTED_MAP.replace("map_", "").capitalize() + " " + (
                    "DM" if self.BATTLE_MODE == 0 else "TDM")
            })
